...
//
// STATES: IDLE -> WRITE_W -> WRITE_A -> COMPUTE -> DONE
//   IDLE    : wait for start.
...
//   DONE    : y valid. cont -> WRITE_A (next vector, weights stay); ~cont -> IDLE.
//
// WEIGHT-STATIONARY: cont loops DONE->WRITE_A, skipping WRITE_W. Weights reload only
//   via IDLE->WRITE_W on a fresh start. Each vector re-clears via WRITE_A's clr pulse.
//
...
//
// Depends on wfull (weight front-end strobe — module not yet built). Serial y_bit
// drain (O4) not handled here.
// ==================================================================================
