...
odbpy_script.py
----------------
Bridges our own core PDN ring out to the padframe template's VSS/VDD pin
locations at the true die edge (see def/A07/project_defs/A/A07_A.def) --
the same problem team D10 solved for their macro with a standalone
connector cell. Runs as the "add-padframe-power-bridge" step of the
librelane_plugin_padframe_bridge Flow, right after OpenROAD.GeneratePDN,
via LibreLane's own OdbpyStep/`openroad -exit -python` machinery -- so
`make librelane-core` produces a self-contained, pad-connected GDS in one
invocation, no separate manual script step.

Why this has to be a step positioned after OpenROAD.GeneratePDN, not raw
Tcl added to PDN_CFG (tried first, then tried again after this plugin was
briefly removed): PDN_CFG is sourced BEFORE pdngen actually generates the
ring's geometry, so at that point there is nothing real to query -- a
hardcoded target has to just hope it lands on continuous ring copper. It
doesn't always: the ring's west (VSS) segment is NOT reliably one
continuous shape end to end -- something placement-dependent (most likely
a routed I/O port pin crossing the ring's track) locally splits it, and
the split's position moves between otherwise-identical runs. A hardcoded
Tcl target that worked on one clean signoff run failed on the very next
run with OpenROAD's own "[PSM-0038] Unconnected shape" / "[PSM-0069]
Check connectivity failed" the moment the split happened to land on top
of it. This script runs after pdngen's real geometry exists and queries
it live, which is the only way to be robust against that.

Two more real bugs were found and fixed while this was still a standalone
script run by hand between two flow invocations (before it became a
proper step); both fixes are preserved here since either can silently
recur if this script is changed carelessly:

1. Landing the bridge at "the midpoint of whichever ring segment happens
   to be biggest" reliably avoids ring gaps and ring-corner via
   congestion, but has nothing to do with where the real padframe pad
   actually is. Both sides reach the die edge on the right layer, but at
   the wrong Y (or X, for VDD) -- so the copper never actually touches
   once merged with the real padframe GDS. Fixed by targeting one of
   A07_A.def's actual pin fingers (VSS_PAD_FINGERS/VDD_PAD_FINGERS below)
   directly, falling back to a best-effort overlap (with a loud warning)
   only if none fits cleanly.

2. A VSS bridge sized off only the VSS ring's own dimensions reached far
   enough in X to overlap VDD's OWN nested west-side ring segment (PDN
   rings are concentric -- both nets have their own vertical Metal2
   segment on the west side, at different X offsets). Magic's extraction
   caught the resulting short directly: "Ports VSS and VDD are
   electrically shorted." Fixed by clipping against the live neighboring-
   net geometry (clip_x1_to_avoid) plus a hard overlap check before
   returning (any future recurrence raises instead of silently shipping
   a short).
"""
...
VSS_PAD_FINGERS_UM200 = [
    (33828, 35728), (31198, 33248), (28828, 30878),
    (26122, 28172), (23752, 25802), (21272, 23172),
]
VDD_PAD_FINGERS_UM200 = [
    (78828, 80728), (76198, 78248), (73828, 75878),
    (71122, 73172), (68752, 70802), (66272, 68172),
]
