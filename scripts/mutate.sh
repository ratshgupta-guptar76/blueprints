...
#
# Usage:  ./scripts/mutate.sh <module>          # from the repo root
# Exit:   0 = every mutation caught;  1 = at least one survived
#
# Mutations live in scripts/mutations/<module>.txt, one per line:
...
if [ -z "$MODULE" ]; then
    echo "Usage: $0 <module>"
    echo "Available:"
    for f in scripts/mutations/*.txt; do
        [ -e "$f" ] || continue
        b="${f##*/}"; echo "  ${b%.txt}"
    done
    exit 2
...
RTL="src/${MODULE}.sv"
TABLE="scripts/mutations/${MODULE}.txt"
